// ============================================================
// 茜特拉莉（Citlali）动态皮肤插件 · Host（独立版 · 可选）
// ------------------------------------------------------------
// 零配置模式请用 client-standalone.js（壁纸与语音已内嵌）。
// 想要原图画质（2560×1440 壁纸 / 800×1159 立绘）时，把本文件内容
// 粘贴到 code.host、client.js 粘贴到 code.client 一起 define/run。
// 客户端会自动检测到本 Host 并切换为 /citlali-skin/ 路由素材。
...
return {
  apply(ctx) {
    ...
    if (fs === undefined || webServer === undefined) {
      console.error('citlali-skin: host services fs/webServer unavailable')
      return
    }
    const cleanup = []
    const cache = new Map()
    ...
    for (const route of ROUTES) {
      try {
        cleanup.push(webServer.register({
          ...
          handler: async (req, res) => {
            try {
              ...
              console.error('citlali-skin: serve failed', route.file, error)
              if (!res.headersSent) {
        ...
        console.error('citlali-skin: route register failed', route.route, error)
      }
    }
    cleanup.push(harness.handle('citlali-urls', () => ({
      walls: ['/citlali-skin/wall1.png', '/citlali-skin/wall2.jpg', '/citlali-skin/wall3.jpg'],
      burst: '/citlali-skin/burst.mp3',
    })))
    cleanup.push(harness.handle('citlali-status', async () => {
      const status = {
        wall1Ok: false, wall2Ok: false, wall3Ok: false, burstOk: false,
        detail: '', voiceText: '灭口交给你们俩了！', skill: '诸曜饬令',
      }
      for (const route of ROUTES) {
        try {
          const bytes = await load(route.file)
    ...
    cleanup.push(harness.handle('citlali-uninstall', async () => {
      const removed = cleanup.splice(0).map((dispose) => { try { dispose() } catch (e) {} })
      console.log('citlali-skin: host 侧素材路由与 RPC 已全部移除（' + removed.length + ' 项）。插件记录仍存在，如需彻底删除请让 AI 执行 cordis_undefine <插件ID>')
